...
假设A*生成的路径是p, 起点记为start, 终点记为dest, 在p外有一个最优解q, 记路径p和q的第一个交点是A, q路径上A的下一个节点是B. 记路径p的总长度为fp(goal),  路径q的总长度为fq(goal). 

由于B是A的邻居，所以B在open list中。

由于路径q优于p, 所以fq(goal) < fp(goal)  (1)

根据一致性原则，路径q在点B处的估计值fq(B) < fq(goal)  (2)

联立(1)(2), 得出fq(B) < fp(goal)

所以，当A*生成的路径p由倒数第二个节点B向终点dest扩展的判断中，由于fq(B) < fp(goal)，所以从open list中取出的节点是路径q上的节点B而不是路径p的终点dest. 由于Q路径上所有的点都满足fq(Bq) < fp(goal), 所以open_list中路径q上的节点将一直排在路径p的终点dest前面，这意味着p路径上的dest在路径q完成之前无法被open list取出。与A\*生成最短路径矛盾。
...
对于图中的每个节点N和N的每个邻居节点N'，如果从节点N移动到节点N'的实际代价是c(N, N')，而启发式函数的估计值为h(N)和h(N')，那么启发式函数h(y)必须满足以下条件：
...
以一个简单的示例来说明生成后继节点的过程，假设当前节点为 `(y, x)`，下面是生成后继节点的伪代码：
...
vector<Node> generateSuccessors(Node currentNode) {
    ...
    // 向上扩展
    if (isValid(y, x - 1)) {
        successors.push_back(Node(y, x - 1));
    }
    ...
    // 向左扩展
    if (isValid(y - 1, x)) {
        successors.push_back(Node(y - 1, x));
    }
...
在上述示例中，`Node` 表示网格地图中的节点，`y` 和 `x` 是节点的坐标。`isValid(y, x)` 是一个辅助函数，用于检查节点 `(y, x)` 是否是一个合法的节点（即是否在地图范围内，并且不是障碍物）。
